...
GitLab API から指定されたプロジェクトとブランチのコミット情報を収集し、データベースに永続化する機能を提供します。

- **エンドポイント**:
  - `GET /commits/collect`: コミット収集ページを表示（プロジェクト一覧を含む）
  - `POST /commits/collect`: コミット収集を実行

- **機能**:
  - プロジェクトとブランチを指定してコミット情報を収集
  - 開始日を指定して、指定日以降のコミットのみを収集（オプション）
  - コミットの統計情報（追加行数、削除行数）を含む
  - ページネーション対応で全コミットを取得
  - レート制限対応（指数バックオフ）

- **使用方法**:
  1. `/commits/collect` にアクセス
  2. プロジェクトを選択
  3. ブランチ名を入力
  4. 開始日を入力（オプション、指定しない場合は全コミットを収集）
  5. 「コミット収集」ボタンをクリック

詳細は `.kiro/specs/gitlab-commit-collection/` を参照してください。
