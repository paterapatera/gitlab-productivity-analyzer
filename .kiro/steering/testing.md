...
### Interface Tests
- **インターフェースのテストは不要**: インターフェース（Port）は型定義のみであり、実装がないためテストを書く必要はない
- **実装のテストで十分**: インターフェースを実装するクラスのテストで、インターフェースの契約が正しく実装されていることを検証する
- **例**: `CommitRepository`インターフェースのテストは不要、`EloquentCommitRepository`のテストで検証

### Service Provider Binding Tests
- **サービスプロバイダーのバインディングテストは不要**: バインディングの設定を検証するテストは書く必要はない
- **動作テストで十分**: 実際の動作テスト（Feature Tests、Unit Tests）で依存性注入が正しく動作することを検証する
- **理由**: バインディングが間違っていれば、実際の動作テストで自然に失敗する。設定の検証ではなく、動作の検証に焦点を当てる
- **例**: `AppServiceProvider`のバインディングテストは不要、`CollectCommitsTest`や`ProjectControllerTest`で検証
